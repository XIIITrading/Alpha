...
// Market update queue management variables
let marketUpdateQueueWarned = false;
...
contextBridge.exposeInMainWorld('electronAPI', {
    ...
    data: {
        /**
         * Connect to the Polygon data server
         * @returns {Promise<boolean>} - True if connected successfully
         */
        connect: () => {
            return ipcRenderer.invoke('data:connect');
        },
         ...
         */
        requestInitialData: () => {
            return ipcRenderer.invoke('data:request-initial');
        },

        /**
         * Get current market status
         * @returns {Promise<string>} - Market status (Open, Closed, Pre-Market, etc.)
         */
        getMarketStatus: () => {
            return ipcRenderer.invoke('data:get-market-status');
        },
 ...
 * Automatically forwards market-data-update events to PerspectiveBridge
 * This ensures seamless data flow from main process to renderer's Perspective tables
 */
(() => {
    // Listen for market-data-update events from main process
    ipcRenderer.on('market-data-update', (event, update) => {
        // Throttle console logging for high-frequency updates
        const now = Date.now();
        const shouldLog = now - lastQueueLogTime > 1000; // Log at most once per second
        
        if (shouldLog) {
            console.log('📊 Received market-data-update:', {
                type: update.type,
                table: update.table,
                dataCount: Array.isArray(update.data) ? update.data.length : 1,
                stream: update.options?.stream
            });
        ...
        if (window.PerspectiveBridge && window.PerspectiveBridge.handleDataUpdate) {
            ...
            if (!window.pendingMarketUpdates) {
                window.pendingMarketUpdates = [];
            }
            
            // Add to queue with size limit
            if (window.pendingMarketUpdates.length < MAX_QUEUE_SIZE) {
                window.pendingMarketUpdates.push(update);
                if (shouldLog) {
                    console.log(`📦 Queued market update (${window.pendingMarketUpdates.length} pending)`);
                }
            } else if (!marketUpdateQueueWarned) {
                console.warn(`⚠️ Market update queue full (${MAX_QUEUE_SIZE} items). Dropping new updates until bridge is ready.`);
                marketUpdateQueueWarned = true;
            }
        }
    });
})();
