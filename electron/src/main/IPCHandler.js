...
class IPCHandler extends EventEmitter {
    ...
    setupDataHandlers() {
        ...
        this.registerHandler('data:request', async (event, { source, params }) => {
            console.log('[IPCHandler] data:request received:', { source, params });
            ...
            try {
                ...
                console.log('[IPCHandler] Generated operationId:', operationId);
                
                // Add to pending operations
                this.pendingOperations.add(operationId);
                
                // Forward request to integration layer (via event)
                console.log('[IPCHandler] Emitting data-request event');
                ...
                console.log('[IPCHandler] Waiting for response...');
        ...
        this.registerHandler('data:subscribe', async (event, { stream, symbols, options }) => {
            console.log('[IPCHandler] 📥 data:subscribe received:', { stream, symbols, options });
            ...
            try {
                ...
                console.log('[IPCHandler] Window ID:', windowId);
                ...
                console.log('[IPCHandler] Created subscription ID:', subscriptionId);
                console.log('[IPCHandler] 📤 Emitting data-subscribe event to PolygonBridge');
                ...
                console.log('[IPCHandler] ✅ Subscription request forwarded');
    ...
    registerHandler(channel, handler) {
        ...
        const wrappedHandler = async (event, ...args) => {
            // Log all data: channel handlers
            if (channel.startsWith('data:')) {
                console.log(`[IPCHandler] 📨 Handling: ${channel}`, args.length > 0 ? args[0] : '(no args)');
            }
            ...
            try {
                ...
                // Log successful data: channel completions
                if (channel.startsWith('data:')) {
                    console.log(`[IPCHandler] ✅ ${channel} completed`);
                }
