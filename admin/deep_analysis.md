# Deep Code Analysis Report - Version 2.0

**Generated:** 2025-06-25T00:02:31.715Z
...
#### admin\deep_analyzer.js

**extractFunctionWithReturn() returns:**
```javascript
{
  "name": "name",
  "params": "params",
...
**extractVariableFunctionWithReturn() returns:**
```javascript
{
  "name": "name",
  "params": "params",
...
**extractReturnFormat() returns:**
```javascript
returns.length > 0 ? returns[0] : null
```

**extractConfiguration() returns:**
```javascript
{
  "name": "name",
  "values": "values"
...
**extractObjectLiteral() returns:**
```javascript
result
```

#### electron\config\window.config.js

**getWindowConfig() returns:**
```javascript
config
```
...
**wrappedHandler() returns:**
```javascript
result
```

#### electron\src\renderer\bridge\core\initialization.js

**loadPerspective() returns:**
```javascript
boolean
```

#### electron\src\renderer\bridge\dashboards\premarket\index.js

**createPreMarketDashboard() returns:**
```javascript
dashboard
```

**createPreMarketGrids() returns:**
```javascript
grids
```

**initializePreMarketModules() returns:**
```javascript
modules
```

#### electron\src\renderer\bridge\dashboards\utils.js

**getDashboardState() returns:**
```javascript
BridgeState.dashboards.get()
```

**isDashboard() returns:**
```javascript
BridgeState.dashboards.has()
```

#### electron\src\renderer\bridge\data\tableManager.js
...
#### electron\src\renderer\bridge\state\bridgeState.js
...
**getMarketData() returns:**
```javascript
BridgeState.marketData.get()
```

**getAllMarketData() returns:**
```javascript
BridgeState.marketData
```
...
#### electron\src\renderer\bridge\viewers\viewerManager.js

**createViewer() returns:**
```javascript
viewer
```
...
**getViewer() returns:**
```javascript
BridgeState.viewers.get()
```

**getViewerIds() returns:**
```javascript
Array.from()
```

#### electron\src\renderer\grid-manager\comparators\percentChange.js

**percentChangeComparator() returns:**
```javascript
Math.abs(valueB || 0) - Math.abs(valueA || 0)
```

#### electron\src\renderer\grid-manager\formatters\currency.js

**currencyFormatter() returns:**
```javascript
string
```

#### electron\src\renderer\grid-manager\formatters\datetime.js
...
#### electron\src\renderer\grid-manager\formatters\largeNumber.js

**largeNumberFormatter() returns:**
```javascript
string
```

#### electron\src\renderer\grid-manager\formatters\percent.js

**percentFormatter() returns:**
```javascript
string
```

#### electron\src\renderer\grid-manager\test\mockRendererEnvironment.js

**createMockParams() returns:**
```javascript
{
  "value": "value",
  "colDef": {
...
#### polygon\api_validator.py

**validate_all_features() returns:**
```javascript
Dict[str, Any]
```

**validate_single_feature() returns:**
```javascript
Dict[str, Any]
```

**_is_cache_valid() returns:**
```javascript
bool
```

**_validate_authentication() returns:**
```javascript
Dict[str, Any]
```

**_validate_tickers() returns:**
```javascript
Dict[str, Any]
```
...
**sync_session() returns:**
```javascript
requests.Session
```

**get_async_session() returns:**
```javascript
aiohttp.ClientSession
```

**_check_rate_limits() returns:**
```javascript
None
```

**_record_request() returns:**
```javascript
None
```

**_prepare_request() returns:**
```javascript
Dict[str, Any]
```
...
**is_retryable_error() returns:**
```javascript
bool
```

**get_retry_delay() returns:**
```javascript
int
```
...
**_fetch_from_cache() returns:**
```javascript
Optional[pd.DataFrame]
```

**_fetch_from_api() returns:**
```javascript
pd.DataFrame
```

**_validate_and_clean() returns:**
```javascript
pd.DataFrame
```

**_fill_data_gaps() returns:**
```javascript
pd.DataFrame
```

#### polygon\polygon_server\server.py
...
#### polygon\polygon_server\utils\json_encoder.py
...
**polygon_json_dumps() returns:**
```javascript
str
```

**polygon_json_response() returns:**
```javascript
dict
```

#### polygon\rate_limiter.py
...
**wait_if_needed() returns:**
```javascript
float
```

**estimate_time_for_requests() returns:**
```javascript
Dict[str, Any]
```
...
**_get_cache_filepath() returns:**
```javascript
Path
```

**_calculate_checksum() returns:**
```javascript
str
```
...
**parse_timeframe() returns:**
```javascript
Tuple[int, str]
```

**validate_symbol() returns:**
```javascript
str
```
...
**format_date_for_api() returns:**
```javascript
str
```

**timestamp_to_datetime() returns:**
```javascript
datetime
```

#### polygon\validators\anomalies.py

**detect_price_anomalies() returns:**
```javascript
Dict[str, Any]
```

**validate_volume_profile() returns:**
```javascript
Dict[str, Any]
```

#### polygon\validators\api_features.py
...
#### polygon\validators\data_quality.py
...
**generate_validation_summary() returns:**
```javascript
Dict[str, Any]
```

#### polygon\validators\gaps.py
...
**is_market_closure_gap() returns:**
```javascript
bool
```

#### polygon\validators\market_hours.py

**validate_market_hours_data() returns:**
```javascript
DataQualityReport
```

#### polygon\validators\ohlcv.py

**validate_ohlcv_integrity() returns:**
```javascript
DataQualityReport
```

**validate_data_continuity() returns:**
```javascript
Dict[str, Any]
```

#### polygon\validators\symbol.py

**validate_symbol_detailed() returns:**
```javascript
Dict[str, Any]
```
...
**_process_trade() returns:**
```javascript
Dict
```

**_process_quote() returns:**
```javascript
Dict
```

**_process_aggregate() returns:**
```javascript
Dict
```
...
**get_latest_price() returns:**
```javascript
float
```

**get_latest_bars() returns:**
```javascript
'pd.DataFrame'
```

**validate_ticker() returns:**
```javascript
bool
```
...
**return value** *(admin\deep_analyzer.js)*
```javascript
{
  "name": "name",
...
**package-lock.json** *(electron\package-lock.json)*
```javascript
{
  ...
  "packages": {
    ...
    "node_modules/electron-devtools-installer": {
      "version": "3.2.1",
      "resolved": "https://registry.npmjs.org/electron-devtools-installer/-/electron-devtools-installer-3.2.1.tgz",
...
**return value** *(electron\src\main\AppUpdater.js)*
```javascript
{
  ...
  "config": "this.config",
  "stats": {
    "lastCheck": "this.state.lastCheck ? new Date(this.state.lastCheck).toISOString() : 'Never'",
    "nextCheck": "this.checkTimer && this.state.lastCheck ? new Date(this.state.lastCheck + this.config.checkInterval).toISOString() : 'N/A'",
...
**return value** *(electron\src\main\DataTransformationService\calculators\ChangeCalculator.js)*
```javascript
{
  "momentum": "momentum",
  "velocity": "velocity",
...
**return value** *(electron\src\main\DataTransformationService\calculators\VolumeCalculator.js)*
```javascript
{
...
**return value** *(electron\src\main\DataTransformationService\index.js)*
```javascript
{
...
**return value** *(electron\src\main\DataTransformationService\MarketDataStore.js)*
```javascript
{
  "current": "this.currentData.get()",
  "history": "this.historyData.get()",
  "previousClose": "this.previousClose.get()",
  "volumeHistory": "this.volumeHistory.get()"
}
```

**return value** *(electron\src\main\IPCHandler.js)*
```javascript
{
  "success": "boolean",
  "error": "error.message",
...
**columns** *(admin\deep_analyzer.js)*
```javascript
[]
```
...
**windowConfig** *(electron\config\window.config.js)*
```javascript
{
  "defaults": {
    "width": 1200,
...
**Grid: columns** *(admin\deep_analyzer.js)*
```javascript
[]
```

**PREMARKET_GRID_CONFIGS** *(electron\src\renderer\bridge\config\premarketConfigs.js)*
```javascript
{
  "gappers": {
    "name": "Gap Scanner",
...
**GRID_DEFAULTS** *(electron\src\renderer\grid-manager\config\gridDefaults.js)*
```javascript
{
  "defaultColDef": {
    "sortable": true,
...
**configTypes** *(admin\deep_analyzer.js)*
```javascript
{
  "app": [],
  "trading": [],
...
**config** *(electron\config\window.config.js)*
```javascript
{
  "webPreferences": {}
}
...
**defaultConfig** *(electron\src\main\StateManager.js)*
```javascript
{
...
**TABLE_CONFIGS** *(electron\src\renderer\bridge\config\tableConfigs.js)*
```javascript
{
  "scanner": {
...
*... and 7 more other configurations*

### Constants and Settings

**DEFAULT_LIMIT:** `1000`
*File: polygon\config.py*
...
1. MARKET DATA INGESTION
   Polygon WebSocket → Python Server → Data Validation
   ↓
2. DATA TRANSFORMATION
   Raw Quotes/Trades → Aggregated Bars → Calculated Metrics
   ↓
3. TRADING ANALYSIS
   Gap Detection → Momentum Calculation → Volume Analysis
   ↓
4. SIGNAL GENERATION
   Condition Checks → Threshold Comparison → Alert Creation
   ↓
5. UI UPDATE
   IPC Message → Grid Update → Visual Indicators
```